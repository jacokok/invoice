<script lang="ts">
	...
	import { getPdf } from "./pdf.remote";
	import { page } from "$app/state";

	const data = $derived(await getPdf());
	...
	const themeParam = $derived(page.url.searchParams.get("theme") ?? "light");
	const primaryColor = $derived(
		themeParam == "light" ? data.project.primaryColorLight : data.project.primaryColorDark
	);
